...
static bool isValidCred(const WifiCred &c)
{
  return (c.ssid && c.ssid[0] != '\0');
}

static void advanceToNextWifi()
{
  ...
  wifiIndex = (wifiIndex + 1) % N;
  wifiFailCountForThis = 0;
...
static void startWifiAttempt()
{
  ...
  // find next valid credential (skip empty slots)
  for (size_t tries = 0; tries < N; tries++)
  {
    if (isValidCred(WIFI_LIST[wifiIndex]))
      break;
    wifiIndex = (wifiIndex + 1) % N;
  }
  ...
  if (!isValidCred(WIFI_LIST[wifiIndex]))
  {
    Serial.println("[WIFI] No valid SSID slots set in .env");
    return;
  }

  Serial.printf("[WIFI] Trying SSID slot %u: %s (attempt %u/%u)\n",
                (unsigned)wifiIndex + 1,
                WIFI_LIST[wifiIndex].ssid,
                (unsigned)wifiFailCountForThis + 1,
                (unsigned)WIFI_FAILS_BEFORE_NEXT);
...
static bool tickWiFiManager()
{
  if (WiFi.status() == WL_CONNECTED)
    return true;

  // If not attempting, start
  if (!wifiAttempting)
  {
    startWifiAttempt();
  ...
  // attempting: check timeout
  if (millis() - wifiAttemptStartMs >= WIFI_JOIN_TIMEOUT_MS)
  {
    wifiAttempting = false;
    wifiFailCountForThis++;

    Serial.printf("[WIFI] Join timeout on SSID slot %u (fail %u/%u)\n",
                  (unsigned)wifiIndex + 1,
                  (unsigned)wifiFailCountForThis,
                  (unsigned)WIFI_FAILS_BEFORE_NEXT);

    if (wifiFailCountForThis >= WIFI_FAILS_BEFORE_NEXT)
    {
      Serial.println("[WIFI] Too many failures, moving to next SSID");
      advanceToNextWifi();
    }
  }
...
static bool tryConnectMQTTOnce()
{
  if (client.connected())
    return true;
  ...
  mqttFailCount++;
  Serial.printf("failed rc=%d (fail %u/%u)\n",
                client.state(), mqttFailCount, MQTT_FAILS_BEFORE_WIFI_DROP);

  // Treat repeated MQTT failures as "Connected but no internet / broker unreachable"
  if (mqttFailCount >= MQTT_FAILS_BEFORE_WIFI_DROP)
  {
    Serial.println("[MQTT] Too many failures. Dropping WiFi and cycling networks...");
...
void loop()
{
  // 1) WiFi manager always runs (cycles if disconnected)
  tickWiFiManager();
  ...
  if (millis() - lastTelemetryMs > 2000)
  {
    ...
    if (lastTelemetryMs != 0)
      dtHours = (nowMs - lastTelemetryMs) / 1000.0f / 3600.0f;
